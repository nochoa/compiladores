...
//        getRootPane().add(scrollPane, BorderLayout.CENTER);
	...
	private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
			...
			if ( this.nalfabeto.getText().isEmpty() || this.talfabeto.getText().isEmpty()){
	            ...
	            JOptionPane.showMessageDialog(null, "El Alfabeto solo puede tener 26 caracteres.","Error",JOptionPane.ERROR_MESSAGE);
			}
			else if (!this.talfabeto.getText().matches("[a-zA-Z0-9]+")) {
				JOptionPane.showMessageDialog(null, "El Alfabeto solo puede contener [a-z], [A-Z] ó [0-9].","ERROR",JOptionPane.ERROR_MESSAGE);         
			}
			else{
				...
				this.agregarFilaAlfabeto(nomalfabeto, alfab);
    ...
    public void agregarFilaAlfabeto(String expNombre2, String expr) {
            ...
            for (int i = 0; i < columnas; i++) {
                for (int j = 0; j < filas; j++) {
                    ...
                    if (((modelo.getValueAt(j, i))).equals(fila[0])) {
                        agregar = false;
                        JOptionPane.showMessageDialog(null,"Ya tiene ese alfabeto","Error", JOptionPane.ERROR_MESSAGE);
                        break;
                    }
                }
                if (agregar == false) {
    ...
    private void cbAlfabetoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ClasesActionPerformed

        int index = this.cbAlfabeto.getSelectedIndex();
        if (index == 1) {
            this.talfabeto.setText("ABCDEFGHIJKLMNOPQRSTUVWXYZ");
            this.nalfabeto.setText("LETRA");
            this.talfabeto.setEnabled(false);
            this.nalfabeto.setEnabled(false);
            
        } else if (index == 2) {
            this.talfabeto.setText("0123456789");
            this.nalfabeto.setText("DIGITO");
            this.talfabeto.setEnabled(false);
            this.nalfabeto.setEnabled(false);
            
        } else if (index == 3){ //Personalizado
    ...
    private void jButtonEliminarActionPerformed(java.awt.event.ActionEvent evt)
    {
	    try
	    {
		    DefaultTableModel temp = (DefaultTableModel)this.tablaAlf.getModel();
    ...
    private void jBaddExprActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
		...
		if ( this.tladoizq.getText().isEmpty() || this.tladoder.getText().isEmpty()){
			JOptionPane.showMessageDialog(null,"Complete todos los campos","Error", JOptionPane.ERROR_MESSAGE);
		}
        else{
	...
	private void jButtonEliminarExpActionPerformed(java.awt.event.ActionEvent evt)
	{
	    try
	    {
